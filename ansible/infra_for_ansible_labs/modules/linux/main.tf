...
resource "azurerm_linux_virtual_machine" "linux_vm" {
  count                 = var.count_val
  name                  = "${var.vm.name}-${format("%1d", count.index + 1)}"
  resource_group_name   = var.resource_group
  location              = var.location
  size                  = var.vm.size
  ...
  os_disk {
    name                 = "${var.vm.name}-${format("%1d", count.index + 1)}-osdisk"
...
resource "azurerm_network_interface" "network_interface" {
  count               = var.count_val
  name                = "${var.vm.name}-${format("%1d", count.index + 1)}-nic"
  location            = var.location
  resource_group_name = var.resource_group
  tags                = local.lab03_tags
  ...
  ip_configuration {
    name                          = "${var.vm.name}-${format("%1d", count.index + 1)}-ip"
...
resource "azurerm_public_ip" "public_ip" {
  count               = var.count_val
  name                = "${var.vm.name}-${format("%1d", count.index + 1)}-pip"
  resource_group_name = var.resource_group
  location            = var.location
  allocation_method   = "Dynamic"
  depends_on          = [var.resource_group]
  domain_name_label   = "${var.vm.name}-${format("%1d", count.index + 1)}"
  tags                = local.lab03_tags
